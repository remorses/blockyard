...
class Participant {
	...
	constructor({
		...
		if (callType === 'audioOnly') {
			...
			this.preview = document.getElementById('screen') as HTMLVideoElement;
		}
		if (!this.preview) {
...
function exampleTransport({ roomId }): TwoWayMessaging {
	...
	const onmessage = async (message: ServerMessage) => {
		switch (message.action) {
				...
				await new Promise(resolve => {
					...
					waitingForResponse.set(
						...
						async (consumerOptions: ConsumerOptions) => {
							...
							producerIdToTrack.set(message.producerId, consumer.track);
							getOrCreateParticipant(message.participantId).addTrack(
								consumer.track
							);
			...
			case 'ProducerRemoved': {
				...
				const participant = getOrCreateParticipant(message.participantId);
...
startCall({
	callType: 'audioOnly',
	preview: document.getElementById('preview-send') as any,
	...exampleTransport({ roomId }),
});
startCall({
	callType: 'shareScreen',
	preview: document.getElementById('screen') as any,
	...exampleTransport({ roomId: roomId.slice(0, -1) + '1' }),
});
